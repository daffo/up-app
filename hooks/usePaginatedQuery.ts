...
export function usePaginatedQuery<T, C>(
  ...
  const fetchPage = useCallback(async (mode: 'initial' | 'refresh' | 'more' | 'silent') => {
    ...
    try {
      ...
      if (mountedRef.current && fetchId === fetchIdRef.current) {
        ...
        cursorRef.current = result.data.length > 0 ? getCursorRef.current(result.data) : undefined;
      }
    } catch (err) {
  ...
  return { data, loading, loadingMore, error, refreshing, hasMore, refresh, loadMore };
}
