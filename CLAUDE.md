...
- **All user-facing text must be localized** - use `t('key')` from `useTranslation()`, including accessibility labels. Add keys to both `locales/en.json` and `locales/it.json`
  - **Italian is always in scope.** Every EN string change (add/edit/remove) must land the matching `it.json` change in the SAME edit — never English-only, never a TODO. Translate idiomatically (not literal); proper nouns / badge names get real Italian equivalents (e.g. Sandstone → Arenaria), not transliterations. When designing a feature, include the IT copy in the spec.
