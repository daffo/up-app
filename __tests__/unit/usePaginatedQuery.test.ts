...
describe('usePaginatedQuery', () => {
  ...
  it('fetches initial page and sets data', async () => {
    ...
    const fetcher = jest.fn().mockResolvedValue({ data: items, hasMore: false });
  ...
  it('loads more and appends data', async () => {
    ...
    expect(fetcher).toHaveBeenLastCalledWith({ created_at: '2024-01-02', id: '1' });
  ...
  it('guards loadMore when already loading more', async () => {
    ...
    const fetcher = jest.fn()
      .mockResolvedValueOnce({ data: [{ id: '1', created_at: '2024-01-01' }], hasMore: true })
  ...
  it('guards loadMore when hasMore is false', async () => {
    const fetcher = jest.fn()
      .mockResolvedValue({ data: [{ id: '1', created_at: '2024-01-01' }], hasMore: false });
  ...
  it('subscribes to cache events and resets on invalidation', async () => {
    ...
    await act(async () => { invalidationCallback(); });
    await flushPromises();
  ...
  it('discards stale responses via fetchIdRef', async () => {
    ...
    const fetcher = jest.fn()
      ...
      .mockResolvedValueOnce({ data: [{ id: 'fresh', created_at: '2024-01-01' }], hasMore: false });
    ...
    await act(async () => {
      resolveFirst!({ data: [{ id: 'stale', created_at: '2024-01-01' }], hasMore: false });
    });
    await flushPromises();
